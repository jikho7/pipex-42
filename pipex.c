...
/*   Updated: 2023/05/06 13:07:45 by jdefayes         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	child_process_0_2(t_pipe d, char *cmd, char *first_arg)
{
	...
	{
		close_pipes(d, 1);
		d.cmd_arg = ft_split(cmd, ' ', 0);
		cmd_path = get_cmd_path(d.cmd_arg[0], d);
		...
		if(execve(cmd_path, d.cmd_arg, d.envp) == -1)
			handle_exec_err(d.fd_pipe2[1], d.cmd_arg, cmd_path);
	}
	close_pipes(d, 3);
	pipe(d.fd_pipe1);
}
...
void	child_process_1_2(t_pipe d, char *cmd, char *last_arg)
{
	...
	if (fork_pid == 0)
	{
		...
		if(dup2(d.fd_pipe2[0], STDIN_FILENO) == -1)
			handle_dup_err(0, d.fd_pipe2[0], d.cmd_arg, cmd_path, 1);
		if (cmd == last_arg)
			last_cmd(d, cmd_path, 1);
		else
			middle_cmd(d, cmd_path, 1);
		if(execve(cmd_path, d.cmd_arg, d.envp) == -1)
			handle_exec_err(d.fd_pipe1[1], d.cmd_arg, cmd_path);
	}
	close_pipes(d, 4);
	pipe(d.fd_pipe2);
}
...
void first_cmd(t_pipe d, char *cmd_path)
{
	...
	if(dup2(d.fd_in, STDIN_FILENO) == -1 || dup2(d.fd_pipe2[1], STDOUT_FILENO) == -1)
	 	handle_dup_err(d.fd_in, d.fd_pipe2[1], d.cmd_arg, cmd_path, 0);
	close_pipes(d, 2);
	close(d.fd_in);
}
...
void last_cmd(t_pipe d, char *cmd_path, int process)
{
	...
	if (process == 0)
	{
		if(dup2(d.fd_pipe1[0], STDIN_FILENO) == 1 || dup2(d.fd_out, STDOUT_FILENO) == - 1)
			handle_dup_err(d.fd_out, d.fd_pipe1[0], d.cmd_arg, cmd_path, 0);
		close_pipes(d, 2);
	}
	if (process == 1)
	{
		if(dup2(d.fd_out, STDOUT_FILENO) == -1)
			handle_dup_err(d.fd_out, d.fd_pipe2[0], d.cmd_arg, cmd_path, 0);
		close_pipes(d, 1);
	}
	// test
	close(d.fd_out);
...
void middle_cmd(t_pipe d, char *cmd_path, int process)
{
	if (process == 0)
	{
		if (dup2(d.fd_pipe1[0], STDIN_FILENO) == -1)
			handle_dup_err(0, d.fd_pipe1[0], d.cmd_arg, cmd_path, 1);
		if (dup2(d.fd_pipe2[1], STDOUT_FILENO) == -1)
			handle_dup_err(1, d.fd_pipe2[1], d.cmd_arg, cmd_path, 1);
		close_pipes(d, 2);
	}
	if (process == 1)
	{
		if(dup2(d.fd_pipe1[1], STDOUT_FILENO) == -1)
			handle_dup_err(1, d.fd_pipe1[1], d.cmd_arg, cmd_path, 1);
		close_pipes(d, 1);
	}
}
